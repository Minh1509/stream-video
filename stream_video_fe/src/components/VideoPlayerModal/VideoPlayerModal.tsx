...
export default function VideoPlayerModal({ video, onClose }: Props) {
  ...
  const { qualities, currentLevel, playerError, changeQuality } = useHlsPlayer(videoRef, video)
      ...
      >
        ...
        <div ref={stageRef} className="relative aspect-video w-full bg-black">
          <video
            ...
            className={`h-full w-full ${playerError ? 'hidden' : ''}`}
          >
            Trình duyệt của bạn không hỗ trợ thẻ video.
          </video>

          {playerError && (
            <div className="absolute inset-0 flex items-center justify-center bg-slate-950 p-6 text-center text-white text-sm">
              {playerError}
            </div>
          )}

          {qualities.length > 1 && !playerError && (
            <div className="absolute left-3 top-3 z-10">
              ...
              {menuOpen && (
                <div className="absolute left-0 top-11 min-w-[120px] overflow-hidden rounded-xl bg-black/85 py-1 text-sm text-white shadow-xl backdrop-blur">
                  <QualityOption label="Auto" active={currentLevel === -1} onClick={() => { changeQuality(-1); setMenuOpen(false) }} />
                  {qualities.map((q) => (
                    <QualityOption
                      key={q.index}
                      label={`${q.height}p`}
                      active={currentLevel === q.index}
                      onClick={() => { changeQuality(q.index); setMenuOpen(false) }}
                    />
                  ))}
                </div>
              )}
...
function QualityOption({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`flex w-full items-center justify-between gap-3 px-4 py-2 text-left transition hover:bg-white/15 ${active ? 'font-semibold text-indigo-300' : 'text-white'}`}
    >
      {label}
      {active && (
